...
if ( ! class_exists( 'Messages_List_Table' ) ) {
    class Messages_List_Table extends WP_List_Table {
        ...
        function get_views() {
            ...
            $base_url = admin_url('edit.php?post_type=messages&page=messages_viewer');
        ...
        function get_columns() { 
            return ['cb' => '<input type="checkbox" />', 'messenger_name' => 'From', 'email' => 'Email', 'form_source' => 'Source', 'entry_status' => 'Status', 'post_date' => 'Received']; 
        }

        function get_sortable_columns() { 
            return ['messenger_name' => ['messenger_name', false], 'form_source' => ['form_source', false], 'entry_status' => ['entry_status', false], 'post_date' => ['post_date', true]]; 
        }
        ...
        function column_messenger_name( $item ) {
            ...
            if ($post_status_view === 'trash') {
                $restore_url = wp_nonce_url(admin_url('edit.php?post_type=messages&page=messages_viewer&action=untrash&post_status=trash&entry_id=' . $item['ID']), 'untrash_message_' . $item['ID']);
                $delete_url = wp_nonce_url(admin_url('edit.php?post_type=messages&page=messages_viewer&action=delete&post_status=trash&entry_id=' . $item['ID']), 'delete_message_' . $item['ID']);
                ...
                $view_url = admin_url('edit.php?post_type=messages&page=messages_viewer&action=view&entry_id=' . $item['ID']);
                $trash_url = wp_nonce_url(admin_url('edit.php?post_type=messages&page=messages_viewer&action=trash&entry_id=' . $item['ID']), 'trash_message_' . $item['ID']);
        ...
        function column_entry_status( $item ) { 
            if ( $item['is_blacklisted'] ) { return 'Blocked <span class="status-indicator-dot red-dot" title="This sender is on the blacklist">●</span>'; } 
            switch ($item['entry_status']) { 
                case 'Unread': return 'Unread <span class="status-indicator-dot green-dot" title="Unread Message">●</span>'; 
                case 'Replied': return 'Replied <span class="status-indicator-dot purple-dot" title="You have replied to this message">●</span>'; 
                case 'Read': default: return 'Read <span class="status-indicator-dot blue-dot" title="Message has been read">●</span>'; 
            } 
        }

        function get_bulk_actions() { 
            $post_status = isset($_REQUEST['post_status']) ? $_REQUEST['post_status'] : 'all';
            if ($post_status === 'trash') { return ['untrash' => 'Restore', 'delete' => 'Delete Permanently']; }
            return ['mark_read' => 'Mark as Read', 'mark_unread' => 'Mark as Unread', 'trash' => 'Move to Trash']; 
        }
        ...
        <div class="wrap">
            ...
            <div class="message-viewer-wrap">
                    ...
                    <div class="postbox">
                        <div class="sender-info-header">
                            ...
                            <p><strong>Email:</strong><a href="mailto:<?php echo esc_attr($pod->field('email')); ?>"><?php echo esc_html($pod->field('email')); ?></a>
                                <?php $email_to_check=$pod->field('email');if(is_email_blacklisted($email_to_check)){echo '<span style="margin-left:10px;font-style:italic;color:#888;">(Blacklisted)</span>';}else{$blacklist_nonce=wp_create_nonce('blacklist_email_'.$entry_id);$blacklist_url=admin_url('edit.php?post_type=messages&page=messages_viewer&action=blacklist&entry_id='.$entry_id.'&_wpnonce='.$blacklist_nonce);echo '<a href="'.esc_url($blacklist_url).'" class="blacklist-button">Blacklist</a>';}?>
                            </p>
                ...
                <div class="message-sidebar">
                    ...
                    <div class="postbox"><h2 class="hndle">Actions</h2><div class="inside sidebar-actions"><a href="<?php echo get_delete_post_link($pod->id());?>" class="submitdelete">Trash Entry</a><a href="#">Print</a><hr><?php if(is_email_blacklisted($email_to_check)){echo '<span>Already Blacklisted</span>';}else{$blacklist_nonce=wp_create_nonce('blacklist_email_'.$entry_id);$blacklist_url=admin_url('edit.php?post_type=messages&page=messages_viewer&action=blacklist&entry_id='.$entry_id.'&_wpnonce='.$blacklist_nonce);echo '<a href="'.esc_url($blacklist_url).'" style="color:#a00;">Blacklist this Email</a>';}?></div></div>
                </div>
            </div>
        </div>
        <?php
...
if ( ! function_exists( 'my_custom_message_settings_init' ) ) {
    function my_custom_message_settings_init() {
        register_setting('my_message_settings_group','my_form_email_routing');add_settings_section('email_routing_section','Email Routing',function(){echo '<p>Enter recipient email addresses, separated by commas. Leave a field blank to disable email notifications for that form.</p>';},'my_message_settings_page');add_settings_field('membership_form_recipients','Membership Form',function(){$options=get_option('my_form_email_routing');echo '<input type="text" name="my_form_email_routing[membership]" value="'.esc_attr($options['membership']??'').'" class="regular-text">';},'my_message_settings_page','email_routing_section');add_settings_field('volunteer_form_recipients','Volunteer Form',function(){$options=get_option('my_form_email_routing');echo '<input type="text" name="my_form_email_routing[volunteer]" value="'.esc_attr($options['volunteer']??'').'" class="regular-text">';},'my_message_settings_page','email_routing_section');add_settings_field('contact_form_recipients','Contact Form',function(){$options=get_option('my_form_email_routing');echo '<input type="text" name="my_form_email_routing[contact]" value="'.esc_attr($options['contact']??'').'" class="regular-text">';},'my_message_settings_page','email_routing_section');add_settings_field('newsletter_form_recipients','Newsletter Signups',function(){$options=get_option('my_form_email_routing');echo '<input type="text" name="my_form_email_routing[newsletter]" value="'.esc_attr($options['newsletter']??'').'" class="regular-text">';},'my_message_settings_page','email_routing_section');
        register_setting('my_message_settings_group','my_form_email_templates');add_settings_section('email_template_section','User Confirmation Email Template',function(){echo '<p>Customize the automated email sent to users after they submit a form. Use the available placeholders.</p><p>Available placeholders: <code>{user_name}</code>, <code>{form_source}</code>, <code>{submitted_data}</code></p>';},'my_message_settings_page');add_settings_field('user_email_subject','Subject',function(){$options=get_option('my_form_email_templates');echo '<input type="text" name="my_form_email_templates[subject]" value="'.esc_attr($options['subject']??'Thank you for your submission!').'" class="regular-text">';},'my_message_settings_page','email_template_section');add_settings_field('user_email_body','Body',function(){$options=get_option('my_form_email_templates');$default_body="Hello {user_name},\n\nWe have successfully received your submission from the {form_source}.\n\nFor your records, here is a copy of the information you submitted:\n\n{submitted_data}\n\nSincerely,\nThe Team";echo '<textarea name="my_form_email_templates[body]" rows="10" class="large-text">'.esc_textarea($options['body']??$default_body).'</textarea>';},'my_message_settings_page','email_template_section');
        register_setting('my_message_settings_group','my_recaptcha_settings');add_settings_section('recaptcha_settings_section','Google reCAPTCHA v3 Settings',function(){echo '<p>Enter your Google reCAPTCHA v3 keys here to enable spam protection on all forms. Get keys from the <a href="https://www.google.com/recaptcha/admin/create" target="_blank">reCAPTCHA Admin Console</a>.</p>';},'my_message_settings_page');add_settings_field('disable_recaptcha','Disable reCAPTCHA',function(){$options=get_option('my_recaptcha_settings');$checked=isset($options['disable'])&&$options['disable']==='1'?'checked="checked"':'';echo '<label><input type="checkbox" name="my_recaptcha_settings[disable]" value="1" '.$checked.'> Temporarily disable reCAPTCHA for testing.</label>';},'my_message_settings_page','recaptcha_settings_section');add_settings_field('enable_debug_mode','Enable Debug Mode',function(){$options=get_option('my_recaptcha_settings');$checked=isset($options['debug'])&&$options['debug']==='1'?'checked="checked"':'';echo '<label><input type="checkbox" name="my_recaptcha_settings[debug]" value="1" '.$checked.'> Output diagnostic info to the browser console and page.</label>';},'my_message_settings_page','recaptcha_settings_section');add_settings_field('recaptcha_site_key','Site Key',function(){$options=get_option('my_recaptcha_settings');echo '<input type="text" name="my_recaptcha_settings[site_key]" value="'.esc_attr($options['site_key']??'').'" class="regular-text">';},'my_message_settings_page','recaptcha_settings_section');add_settings_field('recaptcha_secret_key','Secret Key',function(){$options=get_option('my_recaptcha_settings');echo '<input type="text" name="my_recaptcha_settings[secret_key]" value="'.esc_attr($options['secret_key']??'').'" class="regular-text">';},'my_message_settings_page','recaptcha_settings_section');add_settings_field('recaptcha_score_threshold','Score Threshold',function(){$options=get_option('my_recaptcha_settings');echo '<input type="number" step="0.1" min="0.1" max="1.0" name="my_recaptcha_settings[score_threshold]" value="'.esc_attr($options['score_threshold']??'0.5').'" class="small-text"> <p class="description">Submissions with a score below this value will be blocked. Default is 0.5. Try 0.3 if you are losing valid submissions.</p>';},'my_message_settings_page','recaptcha_settings_section');
    }
...
if ( ! function_exists( 'my_custom_message_viewer_menu' ) ) {
    function my_custom_message_viewer_menu() {
        if (isset($_GET['action']) && !isset($_GET['action2']) && isset($_GET['entry_id'])) {
            $entry_id=intval($_GET['entry_id']);$action=sanitize_key($_GET['action']);$redirect_url=admin_url('edit.php?post_type=messages&page=messages_viewer');if(isset($_REQUEST['post_status'])){$redirect_url=add_query_arg('post_status',sanitize_key($_REQUEST['post_status']),$redirect_url);}
            switch ($action) {
                case 'blacklist':if(isset($_GET['_wpnonce'])&&wp_verify_nonce($_GET['_wpnonce'],'blacklist_email_'.$entry_id)){$pod=pods('messages',$entry_id);if($pod->exists()){$email_to_blacklist=$pod->field('email');if($email_to_blacklist&&!is_email_blacklisted($email_to_blacklist)){pods('blacklist')->add(['post_title'=>$email_to_blacklist,'post_status'=>'publish','email_address'=>$email_to_blacklist]);}wp_safe_redirect(admin_url('edit.php?post_type=messages&page=messages_viewer&action=view&entry_id='.$entry_id.'&blacklisted=true'));exit;}}else{wp_die('Security check failed.');}break;
        ...
        if($unread_count=pods('messages',['where'=>['entry_status.meta_value'=>'Unread']])->total_found()){global $menu;foreach($menu as $key=>$item){if(isset($item[2])&&$item[2]==='edit.php?post_type=messages'){$menu[$key][0].=' <span class="awaiting-mod">'.$unread_count.'</span>';break;}}}
        add_submenu_page('edit.php?post_type=messages','All Messages','All Messages','edit_posts','messages_viewer',function(){if(isset($_GET['action'])&&$_GET['action']==='view'){render_message_detail_view();}else{$post_status=isset($_REQUEST['post_status'])&&$_REQUEST['post_status']==='trash'?'trash':'all';$page_title=($post_status==='trash')?'Trashed Messages':'Message Entries';echo '<div class="wrap"><h1>'.esc_html($page_title).'</h1><form method="post">';$list_table=new Messages_List_Table();$list_table->views();$list_table->prepare_items();$list_table->search_box('Search Messages','search_id');$list_table->display();echo '</form></div>';}});
        add_submenu_page('edit.php?post_type=messages','Trash','Trash','edit_posts','edit.php?post_type=messages&page=messages_viewer&post_status=trash');
        add_submenu_page('edit.php?post_type=messages','Blacklist Manager','Blacklist Manager','edit_posts','edit.php?post_type=blacklist');
        add_submenu_page('edit.php?post_type=messages','Form Settings','Settings','manage_options','my_message_settings_page','my_custom_message_settings_page');
        remove_submenu_page('edit.php?post_type=messages','edit.php?post_type=messages');remove_submenu_page('edit.php?post_type=messages','post-new.php?post_type=messages');
    }
...
if ( ! function_exists( 'my_custom_messages_admin_bar_menu' ) ) {
    function my_custom_messages_admin_bar_menu($wp_admin_bar) {
        ...
        $wp_admin_bar->add_node(['id'=>'my_messages_menu','title'=>$title,'href'=>admin_url('edit.php?post_type=messages&page=messages_viewer')]);
        $wp_admin_bar->add_node(['id'=>'all_messages','parent'=>'my_messages_menu','title'=>__('All Messages'),'href'=>admin_url('edit.php?post_type=messages&page=messages_viewer')]);
        $wp_admin_bar->add_node(['id'=>'trash_messages','parent'=>'my_messages_menu','title'=>__('Trash'),'href'=>admin_url('edit.php?post_type=messages&page=messages_viewer&post_status=trash')]);
        $wp_admin_bar->add_node(['id'=>'message_settings','parent'=>'my_messages_menu','title'=>__('Form Settings'),'href'=>admin_url('edit.php?post_type=messages&page=my_message_settings_page')]);
    }
    add_action('admin_bar_menu', 'my_custom_messages_admin_bar_menu', 90);
}

if ( ! function_exists( 'custom_admin_styles_for_messages' ) ) {
    function custom_admin_styles_for_messages() { echo '<style>.status-indicator-dot{font-size:1.5em;line-height:1;vertical-align:middle;margin-right:8px}.column-status_indicator{width:25px;text-align:center;padding-left:0!important}.red-dot{color:#d63638}.green-dot{color:#28a745}.purple-dot{color:#6f42c1}.blue-dot{color:#3498db}#adminmenu a[href="edit.php?post_type=messages"] .awaiting-mod,#wp-admin-bar-my_messages_menu .awaiting-mod{background-color:#d63638;color:#fff;border-radius:10px;font-weight:600;padding:0 7px}</style>'; }
